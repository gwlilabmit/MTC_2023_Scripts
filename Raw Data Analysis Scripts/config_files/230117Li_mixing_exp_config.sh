#!/bin/sh
#########################################

#Change these variables to match what is needed in your project:

Genome="/home/mirae/data/Sequencing/Sequencing_Analysis/Index/bsub_ecoli_cap" #"," Seperated list of bowtie indexes to align to.
Raw_Data_Location="/home/mirae/data/Sequencing/230117Li/Mixing_Experiment/Raw_Reads/" #Folder where the raw data lives.
Save_Folder_Location="/home/mirae/data/Sequencing/230117Li/Mixing_Experiment/" #Folder for where to store all intermediary files.
Trimming=true #Whether or not to trim from the end of the sequence (eg if there is a known seq that can be read into).
Trim_Seq="N" #The sequence to trim if found. 
Bowtie_Args="-m 1 -v 2"

#If using CDS files to make dataframe, list all CDS files and corresponding genome names in correct order here:

CDS_Dir="/home/mirae/data/Sequencing/Sequencing_Analysis/CDS_files/"
CDS_Files="B_subtilis_CDS.txt,capsule_CDS.txt,E_coli_CDS.txt"
CDS_Genomes="NC_000964.3,I53-50-v4N,NC_000913.2"
CDS_Names="bsubtilis,capsule,ecoli"

########################################

Trim_Save=${Save_Folder_Location}trimmed_files/
Trim_End="_trimmed.fastq"
Sorted_Save=${Save_Folder_Location}aligned_and_sorted_files/
Sorted_End="_${Genome##*/}_sorted.bam"
Depth_Save=${Save_Folder_Location}depth_files/
Wig_Save=${Save_Folder_Location}wig_files/
DataFrame_Save=${Save_Folder_Location}dataframe_files/

#######################################

check_make_dir() {
	if [ ! -d "$1" ]; then
		echo "Making Directory $1"
		mkdir $1
	fi
}

check_make_dir "$Trim_Save"
check_make_dir "$Sorted_Save"
check_make_dir "$Depth_Save"
check_make_dir "$Wig_Save"
check_make_dir "$DataFrame_Save"